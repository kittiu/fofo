...
class stock_card_history(osv.osv):
    _name = 'stock.card.history'
    _auto = False
    _order = 'date asc'

    def read_group(self, cr, uid, domain, fields, groupby, offset=0, limit=None, context=None, orderby=False, lazy=True):
        res = super(stock_card_history, self).read_group(cr, uid, domain, fields, groupby, offset=offset, limit=limit, context=context, orderby=orderby, lazy=lazy)
        if context is None:
            context = {}
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'stock_card_history')
        cr.execute("""
            CREATE OR REPLACE VIEW stock_card_history AS (
              SELECT MIN(id) as id,
                move_id,
                location_id,
                company_id,
                ...
                ((SELECT
                    ...
                    stock_move.partner_id AS partner_id
                FROM
                ...
                AS foo
                GROUP BY move_id, location_id, company_id, product_id, product_categ_id, date, price_unit_on_quant, source, partner_id
            )""")
